...
[`givenergy-cli`](https://github.com/dewet22/givenergy-cli) v1.0 ships alongside the library — the first release where it's something I'd actually reach for. The repo had been sitting as a thin wrapper over the v1.x library for the better part of three years; this is its rewrite around the v2.0 client lifecycle, with a real TUI and an operator-facing command surface.

| 4 | 10 days | ~1.7k | 7 min |
|:--:|:--:|:--:|:--:|
| commands | of build | lines of code | of sparkline history |

**`tui` — live terminal dashboard.** `uv run givenergy-cli --host <ip> tui` brings up a tabbed Textual app: a power-flow topology box and an energy-balance ledger on the left, an inverter / battery details column on the right. The topology has animated connector arrows whose speed is proportional to flow magnitude (`sqrt(power_kw)`, so a 4 kW flow reads as twice as fast as a 1 kW one rather than four times); in-box sparklines next to each value show a trailing ~7-minute view of PV, load, EPS and SOC. The energy-balance ledger uses a 60-second windowed imbalance reading rather than the noisy instantaneous value — register banks aren't sampled atomically, so individual flows can be tens of ms apart even in a real steady state — and the 30-minute energy delta is suppressed when the 0.1 kWh quantisation step dominates the signal.

**`export` — register dump to JSON.** Connects, runs `Client.detect()` to discover the plant topology, loads the holding-register config blocks, and writes every register from every discovered device address to a single portable JSON file. Partial captures on timeout are still written with a warning, so even a flaky inverter produces something to share.

**`inspect` — render an exported dump.** Reconstructs the `Plant` from JSON and prints decoded model fields plus per-device raw register tables (decimal + hex). No network needed; works on any machine with the library installed, which makes it the easiest way to share inverter state with someone debugging from a different LAN.

**`capture` — record redacted wire frames.** `uv run givenergy-cli --host <ip> capture -o frames.log -d 60` writes timestamped `<iso> <rx|tx> <hex>` lines to a file for the duration. The library applies redaction before the bytes reach the CLI: serial numbers come through with their prefix letters intact (those carry hardware-family signal — Gen 2 vs Gen 3 vs AIO, dongle vs inverter) and only the digits zeroed. Same byte length, same offsets, so the resulting file is safe to attach to a public GitHub issue without a separate sanitisation pass. Closes [givenergy-cli#4](https://github.com/dewet22/givenergy-cli/issues/4).

**v2.0 client lifecycle uptake.** The TUI's refresh loop and the export command now both ride on `Client.detect()` + `load_config()` + `refresh()` rather than the v1.x single-shot poll, so a mixed-topology plant (three-phase + HV stack + external meter) renders correctly once the library can model it end-to-end. The `Inverter` → `SinglePhaseInverter` rename and the slot-map required-argument change are reflected through every code path that touches inverter fields.

#### Where the code lives

```text
app.py         ████████████████████████████████████  1,097   TUI (topology, ledger, panels)
history.py     ██████████                              282   ring buffers + sparkline
registers.py   ███████                                 214   export / inspect
__main__.py    ████                                    120   typer wiring
capture.py     ██                                       43   wire-frame logger
```

Most of the line count is the TUI's compositional plumbing — the power-flow box alone is ~400 lines because it pre-renders every row with per-cell colour spans rather than letting Rich format on the fly. That's the difference between a 7 Hz redraw feeling smooth and seeing connector arrows stutter.
...
- Bounds-enforcement flip ([#57](https://github.com/dewet22/givenergy-modbus/issues/57)) — the corruption-pattern evidence in [#78](https://github.com/dewet22/givenergy-modbus/issues/78) makes the case for shipping all three enforcement steps together, including bank-level discard in `Plant._commit_bank`. Field-level rejection alone would miss the in-bounds anomalies that account for the majority of observed corruption events. Plus field-validation of the inherited register mappings ([#48](https://github.com/dewet22/givenergy-modbus/issues/48)).
...
The capture tool has already produced its first substantial finding: [#78](https://github.com/dewet22/givenergy-modbus/issues/78) characterises two distinct corruption patterns observed on my own plant — a deterministic 16-field response substitution at the dongle's IR(0-59) read of device 0x32 (Pattern A), and field-targeted zero substitution on specific BMS temperature fields (Pattern B). Both pre-date v2.0; neither is fixable upstream now that GivEnergy firmware updates have stopped. The v2.1 bounds-enforcement flip is the library-side mitigation — anyone running v2.0 in HA today will see these patterns as occasional graph spikes; v2.1 will discard the bad poll cycles cleanly.
...
Honest "not yet" items rather than commitments to a particular release:

- **Energy tab.** The TUI exposes a second "Energy" tab that's currently a placeholder. The plan is a cumulative-energy view of the day's flows alongside the existing instantaneous-power view — a counterpart to the imbalance ledger but in kWh.
- **Write commands.** No CLI surface yet for setting charge targets, slots or modes. The library's inverter-bound command API has them, and the TUI is the obvious home, but the model-specific mixins from [givenergy-modbus#75](https://github.com/dewet22/givenergy-modbus/issues/75) are landing in 2.1 and the cli would rather wait for that scaffolding than build a parallel layer.
- **Reconnect re-entrancy.** The TUI's auto-reconnect leans on the assumption that the previous reader/writer tasks have self-terminated on EOF before a new `connect()` runs. Tracked upstream at [givenergy-modbus#62](https://github.com/dewet22/givenergy-modbus/issues/62); the TODO sits inline in cli's `app.py` so it doesn't get forgotten when that lands.
- **`debug` command group.** `capture` sits at the top level for now; if more diagnostic tools accrue, a `debug` namespace would be the natural home and `capture` could move alongside them without changing what it does.
