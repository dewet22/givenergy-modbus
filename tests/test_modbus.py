...
def test_read_holding_registers(sync_client):
    ...
    sync_client.read_holding_registers(2, 22)
...
def test_read_holding_registers_validates_response_base_register(sync_client):
    """Ensure we read the ranges of known registers."""
    response = MockedReadHoldingRegistersResponse(base_register=33, register_count=22)
    mock_call = Mock(name='execute', return_value=response)
    sync_client.execute = mock_call
    assert sync_client.read_holding_registers(2, 22) == {}


def test_read_holding_registers_validates_response_register_count(sync_client):
    """Ensure we read the ranges of known registers."""
    response = MockedReadHoldingRegistersResponse(base_register=33, register_count=22)
    mock_call = Mock(name='execute', return_value=response)
    sync_client.execute = mock_call
    assert sync_client.read_holding_registers(33, 11) == {}


@pytest.mark.skipif(sys.version_info < (3, 8), reason="requires python3.8 or higher")
def test_read_input_registers(sync_client):
    """Ensure we read the ranges of known registers."""
    response = Mock(name='ReadInputRegistersResponse', base_register=33, register_count=22)
    mock_call = Mock(name='execute', return_value=response)
    sync_client.execute = mock_call
    sync_client.read_input_registers(33, 22)
    assert mock_call.call_count == 1
...
def test_read_input_registers_validates_response_base_register(sync_client):
    """Ensure we read the ranges of known registers."""
    response = MockedReadInputRegistersResponse(base_register=33, register_count=22)
    mock_call = Mock(name='execute', return_value=response)
    sync_client.execute = mock_call
    assert sync_client.read_input_registers(2, 22) == {}


def test_read_input_registers_validates_response_register_count(sync_client):
    """Ensure we read the ranges of known registers."""
    response = MockedReadInputRegistersResponse(base_register=33, register_count=22)
    mock_call = Mock(name='execute', return_value=response)
    sync_client.execute = mock_call
    assert sync_client.read_input_registers(33, 11) == {}


@pytest.mark.skipif(sys.version_info < (3, 8), reason="requires python3.8 or higher")
def test_write_holding_register(sync_client):
    ...
    with pytest.raises(ValueError) as e:
        sync_client.write_holding_register(HoldingRegister.INVERTER_STATE, 5)
