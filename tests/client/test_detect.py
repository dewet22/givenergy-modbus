...
def test_plant_capabilities_from_dict_tolerates_missing_fields():
    ...
    # Partial dict — present keys are honoured, missing keys take defaults.
    partial = {"device_type": Model.HYBRID.value, "meter_addresses": [1, 2]}
    caps = PlantCapabilities.from_dict(partial)
    assert caps.meter_addresses == [1, 2]
    assert caps.lv_battery_addresses == []  # default, not KeyError
