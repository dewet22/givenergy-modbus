...
async def test_load_config_all_in_one_hybrid_three_phase_before_extended_slots():
    """ALL_IN_ONE_HYBRID has BOTH is_three_phase and has_extended_slots true (#293 Slice C1).
    ...
    HYBRID_HV_GEN3 used to be the example here; it left `is_three_phase` on the first
    81xx field capture (hass#295), so ALL_IN_ONE_HYBRID is now the only model holding
    both facts and therefore the only one that can guard the ordering.
    """
    client = _client_with_caps(Model.ALL_IN_ONE_HYBRID)
    with patch.object(client, "_execute_reads", new_callable=AsyncMock) as mock_exec:
        await client.load_config()
    assert _reqs(mock_exec) == [
        _hr(0, 60),
...
async def test_load_config_hybrid_hv_gen3_skips_three_phase_block():
    """HYBRID_HV_GEN3 keeps its 10-slot map but no longer polls the per-phase bank.

    The reclassification's polling consequence (hass#295): a residential HV hybrid gets
    the single-phase config banks plus HR(240) extended slots, and the HR(1000+) block
    — which answers with zeros on this hardware rather than error-responding — is gone.
    """
